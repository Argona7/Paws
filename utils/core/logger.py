...
def logging_setup():
    ...
    logger.add("paws.log", rotation="50 MB", compression="zip", format=format_info, level="TRACE")
    if config.USE_TG_BOT:
        logger.add(lambda msg: send_log_to_telegram(msg), format="{time:YYYY-MM-DD HH:mm:ss} | {level} | {message}",
                   level="INFO")
